...
#        self.btnExcel.clicked.connect(self.make_excel)
    ...
    def open_file(self):
        ...
        if file_name:
            ...
            df1_des = bk.sheets(1).range('B1:m13').options(pd.DataFrame, index=False, header=False, expand='table').value
            print(df1_des)
            ...
            df1_data = bk.sheets(1).range('B14').options(pd.DataFrame, index=False, header=False, expand='table').value
            df1_data = df1_data.round(1)
            print(df1_data)
            
            # 기종명, Lot no 추출
            test_pn = list(set(partNo))
            print('test part no', test_pn, test_pn[0])
            self.part_name.setText(str(test_pn[0]))
            
            test_ln = list(set(LotNo))
            print('test lot no', test_ln, test_ln[0])
            self.lot_no.setText(str(test_ln[0]))
            ...
            comb_3_cond = np.zeros((no_of_comb*4, 3, 40))
            ...
            output_array = np.zeros((no_of_comb, 4, 3, 40))
            ...
            # title 출력
            # title_arr = np.reshape(title_array, (no_of_comb*4, 3, -1))
            ...
            # wb1.save("test_result_3X4_비정형.xlsx") #해당 워크북(엑셀파일) 저장하기
            ...
            wb1.save(os.path.splitext(file_name)[0]+'combi.xlsx')
             #해당 워크북(엑셀파일) 저장하기            
